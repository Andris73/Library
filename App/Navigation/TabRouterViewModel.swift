...
final class TabRouterViewModel: Sendable {
    ...
    private let logger = Logger(subsystem: "com.Library.Library", category: "TabRouterViewModel")
    private let settings = AppSettings.shared

    // MARK: TabValue
    var tabValue: TabValue? = nil {
        willSet {
            applyLibraryTitleFont(type: newValue?.libraryID?.type ?? .audiobooks)
    ...
    var pinnedTabValues: [TabValue] {
        get { settings.pinnedTabValues }
        set { settings.pinnedTabValues = newValue }
    }
    ...
    init() {
        TabEventSource.shared.invalidateTabs
            .sink { [weak self] _ in
        ...
        TabEventSource.shared.enablePinnedTabs
            .sink { [weak self] _ in
                Task { @MainActor [weak self] in
        ...
        PersistenceManager.shared.webSocket.events.librariesChanged
            .sink { [weak self] _ in
                Task { @MainActor [weak self] in
...
private extension TabRouterViewModel {
    ...
    func restoreLastTabValue(scope: PersistenceManager.CustomizationSubsystem.TabValueCustomizationScope) -> Bool {
        ...
        guard let lastTabValue = settings.lastTabValue else {
            return false
        }
    ...
    func didLoad(connectionID: ItemIdentifier.ConnectionID, libraries: [Library: ([TabValue], [TabValue])]) {
        let hiddenLibraries = settings.hiddenLibraries
        ...
        let visibleLibraries = libraries.filter { !hiddenLibraries.contains($0.id) }

        connectionLibraries[connectionID] = visibleLibraries.sorted { $0.index < $1.index }
...
private extension TabRouterViewModel {
    func applyLibraryTitleFont(type: LibraryMediaType) {
        ...
        if type == .audiobooks && settings.enableSerifFont {
