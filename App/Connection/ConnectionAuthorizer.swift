...
struct ConnectionAuthorizer: View {
    private static let logger = Logger(subsystem: "com.Library.Library", category: "ConnectionAuthorizer")
    ...
    func authorize() {
        ...
        func authorizeOpenID() async throws {
            ...
            let session = try await ASWebAuthenticationSession(url: apiClient.openIDLoginURL(verifier: verifier), callback: .customScheme("Library"), completionHandler: urlCallback)
