...
public final actor APIClient: Sendable {
    ...
    public init(connectionID: ItemIdentifier.ConnectionID, credentialProvider: APICredentialProvider) async throws {
        logger = .init(subsystem: "com.Library.LibraryKit", category: "APIClient::\(connectionID)")
        ...
        configuration.httpCookieStorage = LibraryKit.httpCookieStorage
        configuration.urlCache = nil

        session = URLSession(configuration: configuration, delegate: URLSessionDelegate(), delegateQueue: nil)
        session.sessionDescription = "Library APIClient::\(connectionID)"
...
extension APIClient {
    func clearCookies() {
        guard let cookies = LibraryKit.httpCookieStorage.cookies else {
            return
        }

        for cookie in cookies {
            LibraryKit.httpCookieStorage.deleteCookie(cookie)
        }
    }
