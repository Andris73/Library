...
public extension LibraryKit {
    static func globalSearch(query: String, includeOnlineSearchResults: Bool, filter: Bool = false, allowedItemTypes: [ItemIdentifier.ItemType]? = nil) async throws -> [Item] {
        ...
        if filter {
            let audiobookFilter = AppSettings.shared.audiobooksFilter
                ...
                $0.addTask {
                    ...
                    return await withTaskGroup {
                        for library in libraries {
                            $0.addTask {
                                do {
                                    let (audiobooks, authors, narrators, series) = try await ABSClient[connectionID].items(in: library.id, search: query)
