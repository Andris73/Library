...
typealias PersistedConvenienceDownloadRetrieval = LibrarySchema.PersistedConvenienceDownloadRetrieval
typealias PersistedConvenienceDownloadDownloaded = LibrarySchema.PersistedConvenienceDownloadDownloaded
typealias PersistedConvenienceDownloadAssociation = LibrarySchema.PersistedConvenienceDownloadAssociation
...
extension PersistenceManager {
    ...
    public final actor ConvenienceDownloadSubsystem {
        ...
        public static let BACKGROUND_TASK_IDENTIFIER = "com.Library.convenienceDownload"

        let logger = Logger(subsystem: "com.Library.LibraryKit", category: "ConvenienceDownloadSubsystem")
        ...
        var items: [Audiobook] {
            get async throws {
                ...
                case .grouping(let itemID, let retrieval):
                    ...
                    switch itemID.type {
                    case .series:
                        let series = try await ABSClient[itemID.connectionID].series(with: itemID)
                        audiobooks = series.audiobooks
                    case .collection, .playlist:
                        let collection = try await ABSClient[itemID.connectionID].collection(with: itemID)
                    ...
                    case .amount(let count):
                        result = Array(audiobooks[0..<min(count, audiobooks.count)])
                    case .cutoff:
                        result = audiobooks
                    }
